...
, normalSize = (
    ...
    superexpandedresourceArray: ["sheep", "wheat", "wood", "brick", "ore", "brick", "ore", "ore", "ore", "ore", "ore", "ore", "brick", "brick", "brick", "brick", "brick", "sheep", "sheep", "sheep", "sheep", "sheep", "sheep", "wood", "wood", "wood", "wood", "wood", "wood", "wheat", "wheat", "wheat", "wheat", "wheat", "wheat"],
    prob: ["", "", ".", "..", "...", "....", ".....", "", ".....", "....", "...", "..", "."]}, 17.5), 
expandedSize = 16, 
...
, fillTiles = () => {
    ...
    for (var [t,s] of e.entries()) {
        ...
        8 == s.chit ? a.classList.add("high-prob-special") : a.classList.remove("high-prob-special"),
        6 == s.chit ? a.classList.add("high-prob") : a.classList.remove("high-prob"),
...
// Update modal input fields to reflect current state when modal is opened
function populateSettingsModal() {
    // --- Resource Counts ---
    let resourceArr;
    switch (mode) {
        case "normal": resourceArr = state.resourceArray; break;
        case "expanded": resourceArr = state.expandedresourceArray; break;
        case "super_expanded": resourceArr = state.superexpandedresourceArray; break;
        default: resourceArr = state.resourceArray;
    }
    // Count resources
    const count = type => resourceArr.filter(x => x === type).length;
    document.getElementById('num_wood').value = count('wood');
    document.getElementById('num_brick').value = count('brick');
    document.getElementById('num_sheep').value = count('sheep');
    document.getElementById('num_wheat').value = count('wheat');
    document.getElementById('num_ore').value = count('ore');
    // Desert is not in resourceArr, so get from state
    let expectedDesert = 0;
    switch (mode) {
        case "normal": expectedDesert = 19 - resourceArr.length; break;
        case "expanded": expectedDesert = 30 - resourceArr.length; break;
        case "super_expanded": expectedDesert = 37 - resourceArr.length; break;
        default: expectedDesert = 19 - resourceArr.length;
    }
    document.getElementById('num_desert').value = expectedDesert;

    // --- Chit (Number) Counts ---
    let chitArr;
    switch (mode) {
        case "normal": chitArr = state.numArray; break;
        case "expanded": chitArr = state.expandednumArray; break;
        case "super_expanded": chitArr = state.superexpandednumArray; break;
        default: chitArr = state.numArray;
    }
    // Count chits
    const chitCount = n => chitArr.filter(x => x === n).length;
    document.getElementById('num_2').value = chitCount(2);
    document.getElementById('num_3').value = chitCount(3);
    document.getElementById('num_4').value = chitCount(4);
    document.getElementById('num_5').value = chitCount(5);
    document.getElementById('num_6').value = chitCount(6);
    document.getElementById('num_8').value = chitCount(8);
    document.getElementById('num_9').value = chitCount(9);
    document.getElementById('num_10').value = chitCount(10);
    document.getElementById('num_11').value = chitCount(11);
    document.getElementById('num_12').value = chitCount(12);
}
